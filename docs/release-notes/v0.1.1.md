...
- `llmff inspect -g` dry-run validation for compact inline graphs.
- Inline `retrieve(documents=...,top_k=...)` stages for local lexical retrieval.
- Inline `cache(path=...,key=...)` stages for reproducible cache use in CLI pipelines.
