...
current supported binary distribution lane for `llmff` v0.1.3.
...
```text
https://github.com/syndicalt/llmff/releases/tag/v0.1.3
```
...
```powershell
$expected = (Get-Content .\llmff-0.1.3-x86_64-pc-windows-msvc.zip.sha256).Split()[0]
$actual = (Get-FileHash .\llmff-0.1.3-x86_64-pc-windows-msvc.zip -Algorithm SHA256).Hash.ToLower()
...
```powershell
$msi = "llmff-0.1.3-x86_64-pc-windows-msvc.msi"
$expected = (Get-Content "$msi.sha256").Split()[0]
$actual = (Get-FileHash $msi -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected) { throw "checksum mismatch" }

msiexec /i llmff-0.1.3-x86_64-pc-windows-msvc.msi
llmff --version
```

## Unsigned Installer Expectations

The v0.1.3 macOS `.pkg` and Windows `.msi` installers are unsigned.
