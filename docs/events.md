...
| `failure_kind` | string | `run_failed` | Stable failure class. Current values are `manifest_parse`, `io`, `json`, `graph_validation`, `unknown_stage`, `timeout`, `http`, `stage_execution`, `backend`, `config`, `not_implemented`, and `interrupted`. New values are additive compatibility changes. |
...
cache artifact locations, retry counts, token usage, cache hit rate, backend
and timeout error rates, and sanitized failure classifications from
`run_failed`.
