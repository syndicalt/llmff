...
```bash
scripts/smoke-archive.sh --archive dist/llmff-0.1.3-x86_64-unknown-linux-gnu.tar.gz
```
...
```bash
scripts/package-windows-msi.sh --binary target/release/llmff.exe --version 0.1.3 --target x86_64-pc-windows-msvc --out-dir dist --emit-wxs-only
```
...
```bash
dotnet tool restore
scripts/package-windows-msi.sh --binary target/release/llmff.exe --version 0.1.3 --target x86_64-pc-windows-msvc --out-dir dist
```
...
```bash
scripts/package-macos-pkg.sh --binary target/release/llmff --version 0.1.3 --target aarch64-apple-darwin --out-dir dist --emit-payload-only
```

Smoke test a generated macOS installer payload without installing:

```bash
scripts/smoke-macos-pkg.sh --payload-root dist/llmff-0.1.3-aarch64-apple-darwin.pkgroot
```

Smoke test a generated Debian package without root:

```bash
scripts/smoke-deb.sh --deb dist/llmff_0.1.3_amd64.deb
```
